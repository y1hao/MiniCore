...
A URL shortener service built with ASP.NET Core, serving as the baseline application (Phase 0) for the MiniCore project - an educational effort to re-implement ASP.NET Core from scratch.

## Purpose

MiniCore.Web is a fully functional URL shortener that demonstrates core ASP.NET Core concepts including:
...
This application serves as a reference implementation and test baseline for progressively replacing ASP.NET Core's underlying components with custom implementations.
...
Tests are located in the `MiniCore.Web.Tests` project. Run all tests with:
